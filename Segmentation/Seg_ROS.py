...
model_folder = "/workspaces/isaac_ros-dev/SNR-aware/segformer-b3-finetuned-cityscapes-1024-1024"
feature_extractor = SegformerFeatureExtractor.from_pretrained(model_folder)
model = SegformerForSemanticSegmentation.from_pretrained(model_folder, id2label=id2label, label2id=label2id, ignore_mismatched_sizes=True)
model.to(device)
...
class SegFormer(Node):
    def __init__(self):
        ...
        self.seg_sub = self.create_subscription(Image, 'left/image_raw', self.raw_sub_callback, qos_profile=qos_policy)
